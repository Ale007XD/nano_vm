...
BENCHMARK_PROGRAM = Program(
    ...
    steps=[
        Step(id="step_1_llm", type=StepType.LLM, prompt="Analyze field $value", output_key="analysis"),
        Step(id="step_2_tool", type=StepType.TOOL, tool="mock_processor", parameters={"data": "$analysis"}),
        # ИСПРАВЛЕНИЕ: Безопасное выражение без использования запрещенной функции float()
        Step(id="step_3_cond", type=StepType.CONDITION, condition="$value > 0.9", then="step_error", otherwise="step_4_success"),
        Step(id="step_4_success", type=StepType.TOOL, tool="mock_finalize", parameters={}),
        Step(id="step_error", type=StepType.TOOL, tool="unregistered_tool_to_force_error", parameters={})
    ]
)
...
async def process_item(item: dict, vm: ExecutionVM, sem: asyncio.Semaphore, error_log_file, progress, task_id) -> str:
    ...
    for run_idx in range(1, RUNS + 1):
        ...
        with Progress(
            ...
            with open(log_filename, 'w', encoding='utf-8') as error_log_file:
                tasks = [process_item(item, vm, sem, error_log_file, progress, task_id) for item in dataset]
