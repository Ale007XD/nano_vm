"""
tests.test_v070_sprint2
=======================
Sprint 2: ProjectionLayer + GovernedToolExecutor
...
Покрытие:
  ...
  GovernedToolExecutor:
    - allow: tool в policy + capability есть
    - deny: tool не в policy
    - deny: capability отсутствует
    - allow: policy is None (backward compat)
    - allow: required=[] (tool зарегистрирован, без capability требований)
    - is_allowed() bool interface
    - CapabilityDeniedError message содержит tool_name и policy_id

  GovernedRunProgramHandler:
    - allow: все tools в policy → run_program вызывается
    - deny: tool не в policy → error без запуска VM
    - allow: policy is None → run_program вызывается

  Интеграция ProjectionLayer + FSM lifecycle:
    - project_for_llm после шага сохраняет step_outputs
    - project_for_trace включает __step_outputs__
    - project_for_tool фильтрует по capabilities
"""
...
class TestGovernedToolExecutor:
    def test_allow_tool_with_capability(self, policy: PolicySnapshot) -> None:
        ...
        executor.check("send_email", ["email.send"])  # no raise

    def test_allow_tool_all_capabilities(self, policy: PolicySnapshot) -> None:
        executor = GovernedToolExecutor(policy=policy)
        executor.check("send_email", ["email.read_raw", "email.send"])  # no raise
    ...
    def test_allow_no_policy(self) -> None:
        ...
        executor.check("any_tool", ["any.capability"])  # no raise

    def test_allow_tool_no_required_capabilities(self, policy: PolicySnapshot) -> None:
        executor = GovernedToolExecutor(policy=policy)
        executor.check("send_email", [])  # tool зарегистрирован, без требований
...
class TestGovernedRunProgramHandler:
    ...
    async def test_allow_permitted_tools(self, policy: PolicySnapshot) -> None:
        ...
        program_data = self._make_program_data(["send_email", "get_weather"])
    ...
    async def test_deny_unpermitted_tool(self, policy: PolicySnapshot) -> None:
        ...
        program_data = self._make_program_data(["send_email", "exec_shell"])
    ...
    async def test_no_policy_allows_all(self) -> None:
        ...
        program_data = self._make_program_data(["any_tool", "another_tool"])
    ...
    async def test_unknown_tool_name_returns_none(self, policy: PolicySnapshot) -> None:
        handler = GovernedRunProgramHandler(policy=policy)
        store = MagicMock()
        # handle() для другого tool → None (передать дальше по цепочке)
...
# ---------------------------------------------------------------------------
# Интеграция: ProjectionLayer в FSM lifecycle
# ---------------------------------------------------------------------------


class TestProjectionFSMIntegration:
    """
    Проверяет что ProjectionLayer корректно работает
    с StateContext который создаёт ExecutionVM в процессе выполнения.
    """

    def test_project_after_step_output(self, sanitizer: DeterministicSanitizer) -> None:
        """После шага state содержит step_outputs — LLM projection их включает."""
        state = StateContext(
    ...
    def test_tombstone_propagation_in_llm(self, sanitizer: DeterministicSanitizer) -> None:
        """Tombstoned CapabilityRef в step_outputs → TOMBSTONE_SENTINEL в LLM projection."""
        dead_ref = CapabilityRef(ref_id="vault://x", salt="s", is_tombstone=True)
        state = StateContext(
            data={},
            step_outputs={"extract": dead_ref},
        )
        result = sanitizer.project_for_llm(state)
    ...
    def test_state_immutability(self, sanitizer: DeterministicSanitizer) -> None:
        """project() не мутирует исходный StateContext."""
    ...
    def test_empty_state(self, sanitizer: DeterministicSanitizer) -> None:
        """Пустой StateContext проецируется без ошибок."""
        state = StateContext()
